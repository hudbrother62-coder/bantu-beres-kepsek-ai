...
export async function generateStructured({ systemInstruction, parts, seed = "request", maxOutputTokens = 8192, responseSchema, temperature = 0.2, topP = 0.9 }) {
...
export async function generateGroundedText({ systemInstruction, prompt, seed = "grounded-request", maxOutputTokens = 3072, temperature = 0.1 }) {
  const response = await runWithFailover({
    seed,
    timeoutMs: 60_000,
    makeBody: () => ({
