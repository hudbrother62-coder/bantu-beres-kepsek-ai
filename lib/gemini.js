...
function settings(userKey = "") {
  const appKeys = configuredKeys();
  const personalKey = cleanKey(userKey);
  if (!appKeys.length && !personalKey) {
...
async function runWithFailover({ seed, makeBody, timeoutMs, userKey = "" }) {
  ...
  const keys = [
    ...rotatedKeys(config.appKeys, seed).map((candidate) => ({ ...candidate, scope: "application" })),
    ...(config.personalKey ? [{ key: config.personalKey, slot: "personal", scope: "personal" }] : []),
  ];
  const models = [...new Set([config.primaryModel, config.fallbackModel])];
  const attempts = [];
  let lastError;

  // Shared application keys are exhausted before a personal key is used.
  // This keeps each user's key as an account-only safety net when shared quota is unavailable.
  for (const candidate of keys) {
    for (const model of models) {
      for (let retry = 0; retry < 2; retry += 1) {
        try {
          const response = await requestModel({ model, key: candidate.key, body: makeBody(model), timeoutMs });
          return { ...response, metadata: { model, keySlot: candidate.slot, keyScope: candidate.scope, attempts: attempts.length + 1, usage: response.usage } };
        } catch (cause) {
          lastError = cause;
